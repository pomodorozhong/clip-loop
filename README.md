...
| `--tui` | Open an interactive terminal UI: duration and millisecond presets as dropdowns, optional crop-before-loop in **Video options**, booleans as checkboxes, **Video options** and **Audio options** in separate collapsible sections, and file browse buttons. Requires `uv sync --extra tui`. |
