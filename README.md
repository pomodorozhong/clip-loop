...
This creates `.venv`, resolves dependencies from `uv.lock`, and installs the `clip-loop` command into that environment. The `tui` extra adds [Trogon](https://github.com/Textualize/trogon) (and [Textual](https://textual.textualize.io/)) for the interactive setup UI (`clip-loop tui` or `--tui`). For CLI-only use, `uv sync` without the extra is enough.
...
| `--tui` | Open an interactive terminal UI generated by [Trogon](https://github.com/Textualize/trogon) from the CLI options. Same as `clip-loop tui`. Requires `uv sync --extra tui`. |
| `tui` | Subcommand alias for `--tui`. |
